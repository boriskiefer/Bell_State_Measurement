...
Run Cell 1.
Run Cell 2.
Select a Bell state and click Run BSM.
Expected output
...
Review single-qubit basis states.
Build two-qubit computational-basis states.
Introduce Bell states.
Explain entanglement as non-factorability of the joint state.
Show Bell-state preparation using H and CNOT.
Reverse the preparation circuit to obtain Bell-state measurement.
Run the simulator for all four Bell states.
Connect the two-bit result to entanglement swapping.
Discuss why classical communication is required.
Key teaching points
